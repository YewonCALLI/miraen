...
export default function Flame({ position = [0, 1.2, 0] }: { position?: [number, number, number] }) {
  ...
  return (
    <mesh castShadow position={position} rotation={[0, -Math.PI / 4, 0]} scale={0.175}>
...
void main() {
    ...
    pos.y *= 1. + (cos((posXZlen + 0.25) * 3.1415926) * 0.25 + noise(vec2(0, time)) * 0.125 + 
    noise(vec2(position.x + time, position.z + time)) * 0.5) * position.y; // flame height
...
const fragmentShader = `
    ...
    void main() {
        float v = abs(smoothstep(0.0, 0.2, hValue) - 1.);
        ...
        gl_FragColor.rgb = mix(vec3(1.0, 0.9, 0.4), gl_FragColor.rgb, smoothstep(0.0, 0.3, hValue));
